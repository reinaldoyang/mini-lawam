...
def compute_action_losses(
    ...
    # include_rz it covers all continuous motion dimensions (XYZ+RZ).
...
class MiniLaWAM(nn.Module):
    def __init__(self, cfg: MiniLaWAMConfig):
        ...
        if cfg.action_dim != expected_action_dim:
            raise ValueError(
                f"action_dim must be {expected_action_dim} when "
                f"include_rz={cfg.include_rz}, got {cfg.action_dim}"
            )
    ...
    def _action_pred(self, u_t_tok, u_hat_tok, wrist=None, state=None):
        """(u_t, u_hat_T) patch tokens -> raw head output [B,H,action_dim].
