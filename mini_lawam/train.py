...
def main():
    ...
    ap.add_argument(
        ...
        help="Joystick target only: train a 5D [XYZ, RZ, gripper] action chunk "
             "using raw HDF5 action column 5. Deployment remains orientation-locked "
             "unless rollout_ur7e also receives --enable-rz.",
    )
    ap.add_argument(
        "--include-ry", action="store_true",
        help="Joystick target only: also train RY using raw HDF5 action column 4. "
             "Combined with --include-rz this yields a 6D [XYZ, RY, RZ, gripper] "
    ...
    action_dim = 4 + int(args.include_rz) + int(args.include_ry)
